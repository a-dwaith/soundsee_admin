    ...
    data.forEach((request) => {
      ...
      requestElement.innerHTML = `
        <div class="card-body d-flex flex-column">
          ...
          <div class="mt-auto">
          <button type="button" class="btn btn-success acceptButton mr-2">Accept</button>
          <button type="button" class="btn btn-danger rejectButton">Reject</button>
          </div>
        </div>
      `;
      const acceptButton = requestElement.querySelector(".acceptButton");
      const rejectButton = requestElement.querySelector(".rejectButton");

      acceptButton.addEventListener("click", () => {
        acceptUserRequest(request.name, request.udid, requestElement);
      });
      rejectButton.addEventListener("click", () => {
        rejectUserRequest(requestElement);
      });
